...
clampf(float value, float min_inclusive, float max_inclusive)
{
  ...
  return value < min_inclusive   ? min_inclusive
         : value < max_inclusive ? value
                                 : max_inclusive;
}

inline void
normalize_point(float x, float y, Pointf* out)
{
  float n = x * x + y * y;
  // Already normalized.
  if (n == 1.0f) {
...
bool
ParticleSystem::initWithTotalParticles(int numberOfParticles)
{
...
void
ParticleSystem::resetTotalParticles(int numberOfParticles)
{
...
void
ParticleSystem::addParticles(int count)
{
  if (_paused) {
    return;
  }
  ...
  // position
  for (int i = start; i < _particleCount; ++i) {
    particle_data_[i].posx =
      _sourcePosition.x + _posVar.x * RANDOM_M11(&RANDSEED);
  }

  for (int i = start; i < _particleCount; ++i) {
    particle_data_[i].posy =
      _sourcePosition.y + _posVar.y * RANDOM_M11(&RANDSEED);
  }
...
#define SET_DELTA_COLOR(c, dc)                                                 \
  for (int i = start; i < _particleCount; ++i) {                               \
    particle_data_[i].dc = (particle_data_[i].dc - particle_data_[i].c) /      \
                           particle_data_[i].timeToLive;                       \
  }
  ...
  if (_endSize != START_SIZE_EQUAL_TO_END_SIZE) {
    for (int i = start; i < _particleCount; ++i) {
      ...
      particle_data_[i].deltaSize =
        (endSize - particle_data_[i].size) / particle_data_[i].timeToLive;
    }
  } else {
    for (int i = start; i < _particleCount; ++i) {
  ...
  // rotation
  for (int i = start; i < _particleCount; ++i) {
    particle_data_[i].rotation =
      _startSpin + _startSpinVar * RANDOM_M11(&RANDSEED);
  }
  for (int i = start; i < _particleCount; ++i) {
    float endA = _endSpin + _endSpinVar * RANDOM_M11(&RANDSEED);
    particle_data_[i].deltaRotation =
      (endA - particle_data_[i].rotation) / particle_data_[i].timeToLive;
  }
  ...
  if (_emitterMode == Mode::GRAVITY) {
    ...
    // radial accel
    for (int i = start; i < _particleCount; ++i) {
      particle_data_[i].modeA.radialAccel =
        modeA.radialAccel + modeA.radialAccelVar * RANDOM_M11(&RANDSEED);
    }

    // tangential accel
    for (int i = start; i < _particleCount; ++i) {
      particle_data_[i].modeA.tangentialAccel =
        modeA.tangentialAccel +
        modeA.tangentialAccelVar * RANDOM_M11(&RANDSEED);
    }
  ...
  // Mode Radius: B
    ...
    for (int i = start; i < _particleCount; ++i) {
      particle_data_[i].modeB.radius =
        modeB.startRadius + modeB.startRadiusVar * RANDOM_M11(&RANDSEED);
    }

    for (int i = start; i < _particleCount; ++i) {
      particle_data_[i].modeB.angle =
        Deg2Rad(_angle + _angleVar * RANDOM_M11(&RANDSEED));
    }

    for (int i = start; i < _particleCount; ++i) {
      particle_data_[i].modeB.degreesPerSecond =
        Deg2Rad(modeB.rotatePerSecond +
                modeB.rotatePerSecondVar * RANDOM_M11(&RANDSEED));
    }
    ...
    if (modeB.endRadius == START_RADIUS_EQUAL_TO_END_RADIUS) {
      ...
      for (int i = start; i < _particleCount; ++i) {
        float endRadius =
          modeB.endRadius + modeB.endRadiusVar * RANDOM_M11(&RANDSEED);
        particle_data_[i].modeB.deltaRadius =
          (endRadius - particle_data_[i].modeB.radius) /
          particle_data_[i].timeToLive;
      }
    }
  }
...
ParticleSystem::update(float dt)
{
  if (_isActive && _emissionRate) {
    ...
    int emitCount =
      (std::min)(1.0f * (totalParticles - _particleCount), _emitCounter / rate);
  ...
  if (_emitterMode == Mode::GRAVITY) {
    for (int i = 0; i < _particleCount; ++i) {
      ...
      if (particle_data_[i].posx || particle_data_[i].posy) {
        normalize_point(
          particle_data_[i].posx, particle_data_[i].posy, &radial);
      }
      tangential = radial;
    ...
    for (int i = 0; i < _particleCount; ++i) {
      particle_data_[i].modeB.angle +=
        particle_data_[i].modeB.degreesPerSecond * dt;
      particle_data_[i].modeB.radius +=
        particle_data_[i].modeB.deltaRadius * dt;
      particle_data_[i].posx =
        -cosf(particle_data_[i].modeB.angle) * particle_data_[i].modeB.radius;
      particle_data_[i].posy = -sinf(particle_data_[i].modeB.angle) *
                               particle_data_[i].modeB.radius * _yCoordFlipped;
    }
  }
...
void
ParticleSystem::setTexture(SDL_Texture* var)
{
  if (_texture != var) {
    _texture = var;
  }
...
ParticleSystem::draw()
{
  ...
  for (int i = 0; i < _particleCount; i++) {
    ...
    SDL_FRect r = { (float)(p.posx + p.startPosX - p.size / 2),
                    (float)(p.posy + p.startPosY - p.size / 2),
                    (float)p.size,
                    (float)p.size };
    SDL_Color c = { Uint8(p.colorR * 255),
                    Uint8(p.colorG * 255),
                    Uint8(p.colorB * 255),
                    Uint8(p.colorA * 255) };
...
void
ParticleSystem::setTangentialAccel(float t)
{
  modeA.tangentialAccel = t;
}

float
ParticleSystem::getTangentialAccel() const
{
  return modeA.tangentialAccel;
}

void
ParticleSystem::setTangentialAccelVar(float t)
{
  modeA.tangentialAccelVar = t;
}

float
ParticleSystem::getTangentialAccelVar() const
{
  return modeA.tangentialAccelVar;
}

void
ParticleSystem::setRadialAccel(float t)
{
  modeA.radialAccel = t;
}

float
ParticleSystem::getRadialAccel() const
{
  return modeA.radialAccel;
}

void
ParticleSystem::setRadialAccelVar(float t)
{
  modeA.radialAccelVar = t;
}

float
ParticleSystem::getRadialAccelVar() const
{
  return modeA.radialAccelVar;
}

void
ParticleSystem::setRotationIsDir(bool t)
{
  modeA.rotationIsDir = t;
}

bool
ParticleSystem::getRotationIsDir() const
{
  return modeA.rotationIsDir;
}

void
ParticleSystem::setGravity(const Vec2& g)
{
  modeA.gravity = g;
}
...
float
ParticleSystem::getSpeed() const
{
  return modeA.speed;
}

void
ParticleSystem::setSpeedVar(float speedVar)
{
...
float
ParticleSystem::getSpeedVar() const
{
...
void
ParticleSystem::setStartRadius(float startRadius)
{
  modeB.startRadius = startRadius;
}

float
ParticleSystem::getStartRadius() const
{
  return modeB.startRadius;
}

void
ParticleSystem::setStartRadiusVar(float startRadiusVar)
{
  modeB.startRadiusVar = startRadiusVar;
}

float
ParticleSystem::getStartRadiusVar() const
{
  return modeB.startRadiusVar;
}

void
ParticleSystem::setEndRadius(float endRadius)
{
  modeB.endRadius = endRadius;
}

float
ParticleSystem::getEndRadius() const
{
  return modeB.endRadius;
}

void
ParticleSystem::setEndRadiusVar(float endRadiusVar)
{
  modeB.endRadiusVar = endRadiusVar;
}

float
ParticleSystem::getEndRadiusVar() const
{
...
void
ParticleSystem::setRotatePerSecond(float degrees)
{
  modeB.rotatePerSecond = degrees;
}

float
ParticleSystem::getRotatePerSecond() const
{
  return modeB.rotatePerSecond;
}

void
ParticleSystem::setRotatePerSecondVar(float degrees)
{
  modeB.rotatePerSecondVar = degrees;
}

float
ParticleSystem::getRotatePerSecondVar() const
{
  return modeB.rotatePerSecondVar;
}

bool
ParticleSystem::isActive() const
{
  return _isActive;
}

int
ParticleSystem::getTotalParticles() const
{
  return _totalParticles;
}

void
ParticleSystem::setTotalParticles(int var)
{
  _totalParticles = var;
}

bool
ParticleSystem::isAutoRemoveOnFinish() const
{
  return _isAutoRemoveOnFinish;
}

void
ParticleSystem::setAutoRemoveOnFinish(bool var)
{
...
bool
ParticleSystem::isPaused() const
{
  return _paused;
}

void
ParticleSystem::pauseEmissions()
{
  _paused = true;
}
